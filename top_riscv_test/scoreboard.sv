module scoreboard(
  ...
  input  logic i_clk,
  input  logic i_rst_n,
  input  logic [63:0] o_counter, o_instr_cnt
);

  logic [31:0] pc_debug;

  assign pc_debug = o_instr_vld ? o_pc_debug : 32'h0;
  ...
  always_ff @(posedge i_clk) begin
    case (o_pc_debug)
      ...
      32'h166C: begin
        if (o_instr_vld == 1) instr_vld = instr_vld & 1'b0;
      end
      32'h1674: begin
        if (o_instr_vld == 1) instr_vld = instr_vld & 1'b0;
      end
      32'h1680: begin 
        if (instr_vld) $write("PASSED\n"); else $write("FAILED\n"); 
        $display("Cycle count %d", o_counter);
        $display("Instruction count %d", o_instr_cnt);  
      end
